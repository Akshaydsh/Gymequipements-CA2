...
CREATE TABLE plans(name VARCHAR(100), PRIMARY KEY(name), exercise(VARCHAR(100)), sets(VARCHAR(100)), reps(VARCHAR(100)));
...
CREATE TABLE progress(username VARCHAR(200), date DATE, daily_result VARCHAR(200), time TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(username, date), FOREIGN KEY(username) references members(username), rate(VARCHAR(200)) );

CREATE TABLE equip(name VARCHAR(200), count  VARCHAR(200));
...
ALTER TABLE plans ADD COLUMN exercise VARCHAR(200) AFTER name;

ALTER TABLE plans ADD COLUMN sets VARCHAR(200) AFTER exercise;
ALTER TABLE plans ADD COLUMN reps VARCHAR(200) AFTER sets;

ALTER TABLE progress ADD COLUMN rate VARCHAR(200); 